...
void TestGetAsyncKeyState(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int)
{
	WNDCLASSEX wc;
	MSG msg;
	...
	while (true)
	{
		...
		{
			const auto down = GetAsyncKeyState(VK_DOWN);
			const auto up = GetAsyncKeyState(VK_UP);
			const auto left = GetAsyncKeyState(VK_LEFT);
			const auto right = GetAsyncKeyState(VK_RIGHT);
			const auto shift = GetAsyncKeyState(VK_SHIFT);
			const auto control = GetAsyncKeyState(VK_CONTROL);
